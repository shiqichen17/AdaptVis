...
class LlavaMultiModalProjector(nn.Module):
    ...
    def forward(self, image_features):
        ...
        hidden_states = self.head_out(hidden_states)
...
class LlavaForConditionalGeneration(LlavaPreTrainedModel):
    def __init__(self, config: LlavaConfig):
        ...
        self.language_model = llama.LLaMAForCausalLM._from_config(
            LLaMAConfig()
        )
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None and input_ids.shape[1] != 1:
                ...
                inputs_embeds, attention_mask, position_ids, image_ids = self._merge_input_ids_with_image_features(
        ...
        if early_exit_layers:
            ...
            return outputs[0],LlavaCausalLMOutputWithPast(
                loss=loss,
                logits=logits,
                past_key_values=outputs[1].past_key_values,
                hidden_states=outputs[1].hidden_states,
                attentions=outputs[1].attentions,
            )
